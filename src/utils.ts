 ...
 */
export function parseBytes(input: string): ParseResult {
  if (typeof input !== 'string' || input.trim().length === 0) {
    throw new Error('Input must be a non-empty string');
  }

  const trimmed = input.trim();
  // Match: optional sign, number (integer, decimal, or .decimal), optional whitespace, unit
  ...
  const entry = getUnitEntry(unitPart);
  if (!entry) {
    throw new Error(`Unknown unit: "${unitPart}". Supported units: B, KB, MB, GB, TB, PB, KiB, MiB, GiB, TiB, PiB`);
  }
